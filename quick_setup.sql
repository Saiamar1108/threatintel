-- Quick Setup SQL for Threat Intelligence Platform
...
CREATE POLICY "users_manage_own_profiles" ON public.user_profiles FOR ALL TO authenticated USING (id = auth.uid()) WITH CHECK (id = auth.uid());

CREATE OR REPLACE FUNCTION public.get_user_role_from_auth() RETURNS TEXT LANGUAGE sql STABLE SECURITY DEFINER AS $$ SELECT COALESCE((SELECT role::TEXT FROM public.user_profiles WHERE id = auth.uid()), 'analyst') $$;

CREATE POLICY "leadership_full_access_threats" ON public.threats FOR ALL TO authenticated USING (public.get_user_role_from_auth() = 'leadership') WITH CHECK (public.get_user_role_from_auth() = 'leadership');

CREATE POLICY "analysts_manage_threats" ON public.threats FOR ALL TO authenticated USING (public.get_user_role_from_auth() IN ('analyst', 'operations') AND (analyst_assigned = auth.uid() OR created_by = auth.uid() OR analyst_assigned IS NULL)) WITH CHECK (public.get_user_role_from_auth() IN ('analyst', 'operations'));

CREATE POLICY "users_manage_own_investigations" ON public.investigations FOR ALL TO authenticated USING (investigator_id = auth.uid()) WITH CHECK (investigator_id = auth.uid());

CREATE POLICY "users_manage_own_activity_logs" ON public.user_activity_logs FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

CREATE POLICY "authenticated_read_threat_actors" ON public.threat_actors FOR SELECT TO authenticated USING (true);

CREATE POLICY "users_create_threat_actors" ON public.threat_actors FOR INSERT TO authenticated WITH CHECK (created_by = auth.uid());

CREATE POLICY "creators_update_threat_actors" ON public.threat_actors FOR UPDATE TO authenticated USING (created_by = auth.uid()) WITH CHECK (created_by = auth.uid());

CREATE POLICY "analysts_manage_threat_indicators" ON public.threat_indicators FOR ALL TO authenticated USING (public.get_user_role_from_auth() IN ('analyst', 'operations', 'leadership')) WITH CHECK (public.get_user_role_from_auth() IN ('analyst', 'operations', 'leadership'));

CREATE POLICY "authenticated_read_threat_associations" ON public.threat_actor_associations FOR SELECT TO authenticated USING (true);

CREATE POLICY "analysts_manage_threat_associations" ON public.threat_actor_associations FOR INSERT, UPDATE, DELETE TO authenticated USING (public.get_user_role_from_auth() IN ('analyst', 'operations', 'leadership')) WITH CHECK (true);

CREATE POLICY "briefings_role_based_access" ON public.briefings FOR SELECT TO authenticated USING (is_published = true OR created_by = auth.uid() OR public.get_user_role_from_auth() = ANY(target_audience) OR public.get_user_role_from_auth() = 'leadership');

CREATE POLICY "users_manage_own_briefings" ON public.briefings FOR INSERT, UPDATE, DELETE TO authenticated USING (created_by = auth.uid()) WITH CHECK (created_by = auth.uid());

-- 6. Create Functions
CREATE OR REPLACE FUNCTION public.handle_new_user() RETURNS TRIGGER SECURITY DEFINER LANGUAGE plpgsql AS $$ BEGIN INSERT INTO public.user_profiles (id, email, full_name, role, organization) VALUES (NEW.id, NEW.email, COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1)), COALESCE((NEW.raw_user_meta_data->>'role')::public.user_role, 'analyst'::public.user_role), COALESCE(NEW.raw_user_meta_data->>'organization', 'ThreatIntel Corp')); RETURN NEW; END; $$;

CREATE OR REPLACE FUNCTION public.update_updated_at_column() RETURNS TRIGGER SECURITY DEFINER LANGUAGE plpgsql AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$;
...
CREATE TRIGGER update_threat_actors_updated_at BEFORE UPDATE ON public.threat_actors FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
